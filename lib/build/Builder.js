...
blogList().then(s => {
   ...
   * @param {string} targetPath - 文件存放目录
   * @param {pageConfig} pageConfig
   */
  async renderBlog(mdHtml, title, targetPath, pageConfig) {
    ...
    const navList = this.siteConfig.navList.map(nav => {
      const obj = Object.assign({}, nav);
      if (nav.path.indexOf(pageConfig.urlPath) > -1) {
        obj.active = true;
      }
      return obj;
    });
    ...
    const templateHtml = await ejs.renderFile(this.templateEjsPath, {
      content: blogHtml,
      title: title + ' - ' + this.siteConfig.title,
      links: cssAssetsList,
  ...
  async renderListPage(pageConfig) {
    ...
    const templateHtml = await ejs.renderFile(this.templateEjsPath, {
      content: listHtml,
      title: pageConfig.title + ' - ' + this.siteConfig.title,
      links: cssAssetsList,
      scripts: jsAssetsList
    });
  ...
  scanBlog(pageConfig) {
    ...
    for (let i = 0; i < blogList.length; i++) {
      if (blogList[i].isFile() && /\.(md|MD)$/.test(blogList[i].name)) {
        ...
        baseInfo.path = path.join(this.buildConfig.publicPath, pageConfig.urlPath, baseInfo.fileName + '.html');

        blogInfoList.push(baseInfo);
        this.renderBlog(mdHtml, baseInfo.title, targetPath, pageConfig);
      }
    }
