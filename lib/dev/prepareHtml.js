...
async function prepareBlogHtml() {
  ...
  const BLOG_MD_PATH = resolve('blog/技术/Markdown 基本语法.md');
...
async function prepareListHtml() {
  ...
  const listHtml = await ejs.renderFile(LIST_LAYOUT_PATH, {
    blogInfoList: blogInfoList.slice(0, 0) // 先渲染10条数据
  });
  ...
  fs.writeFileSync(resolve('.temp/list-blog-info-list.json'), JSON.stringify(blogInfoList));
