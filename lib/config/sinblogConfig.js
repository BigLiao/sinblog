...
const configScheme = Joi.object({
  ...
  build: Joi.object({
    ...
    blogPage: [
      ...
      Joi.object({
        title: Joi.string().optional().description('导航栏标题'),
        dirPath: Joi.string().required().description('博客目录路径'),
        urlPath: Joi.string().alphanum().optional().description('URL路径'),
      }).description('博客目录配置')
    ],
    publicPath: Joi.string().pattern(/^\/(.+\/$|$)/).description('url 路径')
      .optional().default('/')
  }),
...
function formatConfig(config, rootPath) {
  ...
  if (isMultiPage) {
    ...
    config.site.navList = config.build.blogPage.map(item => ({
      title: item.title,
      path: path.join(config.build.publicPath, item.urlPath)
    }));
    ...
    if (typeof config.build.blogPage === 'string') {
      ...
      item = {
        title: '',
        dirPath: config.build.blogPage.dirPath,
        urlPath: ''
      }
    }
    const pageConfig = {};
